...
def test_mismatch_kernel_initialization(
    ...
    assert mismatch_kernel.K_train_uncentered is None
    assert mismatch_kernel.n_train_samples is None
...
def test_preprocessing_with_mismatches(
    ...
    mismatch_kernel.preprocess_data(test_sequences, is_train=True)
...
def test_cross_gram_matrix_with_mismatches(
    ...
    K_train = mismatch_kernel.compute_gram_matrix(test_sequences, center=False)

    # Then compute cross gram matrix
    K_cross = mismatch_kernel.compute_gram_matrix(
    ...
    similarity = K_cross[1, 0]  # CGCG vs ACGT
    assert similarity >= 0  # Should be non-negative
...
def test_edge_cases_with_mismatches(mismatch_kernel: MismatchSpectrumKernel) -> None:
    ...
    mismatch_kernel.preprocess_data(df_empty, is_train=True)
    assert all(
